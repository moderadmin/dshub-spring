     ...
     switch(Issued_Command.charAt(0))
     {
         ...
         case 'H':
              ...
              {   new STAError(cur_client,100,"MSG Invalid Context H");
                   ...
                   {
                       int index= DSHubInitializer.listaBanate.isOK(message);
                    
                   
                 //  System.out.println(index);
                   if(index!=-1)//not ok
                    if(!((Issued_Command.startsWith("E") || Issued_Command.startsWith("D") ) //if pm
                    		&&  (DSHubInitializer.listaBanate.getPrAt(index) & BannedWord.privatechat )==0))
                    	//and private chat control activated
                   {
                    	System.out.println("ok");
                       long what= DSHubInitializer.listaBanate.getPrAt(index);
                       /* 
                     ...
                     //  long what=56;
                       ...
                       what/=2;
                        if(what % 2 ==1)
                        {
                           ...
                           Issued_Command=Issued_Command.replace(message,ADC.retADCStr(DSHubInitializer.listaBanate.getReplAt(index)));
