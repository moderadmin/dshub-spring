...
public class CommandParser {
	...
	//	String NI = cur_client.NI;
		...
		//	Main.Server.rewritebans();
			...
			DSHubInitializer.PopMsg("Hub is being shut down by " + cur_client.NI);
		...
		//	BanList.First = null;
			...
			DSHubInitializer.PopMsg("Hub restarted by " + cur_client.NI);
		...
		else if(recvbuf.toLowerCase().startsWith("adcs"))
		{
			...
			if(DSHubInitializer.nogui)
       	 {
       		 cur_client.sendFromBot("GUI disabled.");
       		 return;
       	 }
			if (!DSHubInitializer.GUI.isDisplayable()) {
				try {
			...
			if (DSHubInitializer.GUIok)
				if (DSHubInitializer.GUI.isDisplayable() && !DSHubInitializer.GUI.isShowing()) {
					DSHubInitializer.GUI.setVisible(true);
					cur_client.sendFromBot("GUI launched...");
					//GUIok=true;
					DSHubInitializer.PopMsg("GUI was launched by " + cur_client.NI);
					DSHubInitializer.GUI.SetStatus("GUI restored...");
				} else
					cur_client.sendFromBot("GUI not viewable.");
			...
			try {
				...
				if (AccountsConfig.unreg(aux)) {
					...
					DSHubInitializer.PopMsg(cur_client.NI + " deleted the reg " + aux);
					...
					if (temp.cur_client.userok == 1)
						if ((temp.cur_client.NI.toLowerCase().equals(aux
							...
							if (!temp.cur_client.reg.isreg)
								...
								DSHubInitializer.PopMsg(cur_client.NI + " deleted the reg "
										+ temp.cur_client.ID);
								cur_client.sendFromBot("User "
			...
			if (aux.length() == 39) //possible CID, lets try
			{
				try {
					...
					DSHubInitializer.PopMsg(cur_client.NI + " regged the CID " + aux);
						...
						if (temp.cur_client.userok == 1)
							if ((temp.cur_client.NI.toLowerCase().equals(aux
								...
								temp.cur_client.LoggedAt = System
										.currentTimeMillis();
								temp.cur_client.reg.LastIP = temp.cur_client.RealIP;
								DSHubInitializer.PopMsg(cur_client.NI + " regged the CID "
										+ temp.cur_client.ID);
								DSHubInitializer.Server.rewriteregs();
								return;
							}
			...
			String blah = "Death Squad Hub. Version "
					...
					+ ".\n"
					+ "  Running on "
					+ DSHubInitializer.Proppies.getProperty("os.name")
					+ " Version "
					+ DSHubInitializer.Proppies.getProperty("os.version")
					+ " on Architecture "
					+ DSHubInitializer.Proppies.getProperty("os.arch")
					+ "\n"
					+ "  Java Runtime Environment "
					+ DSHubInitializer.Proppies.getProperty("java.version")
					+ " from "
					+ DSHubInitializer.Proppies.getProperty("java.vendor")
					+ "\n"
					+ "  Java Virtual Machine "
					+ DSHubInitializer.Proppies
