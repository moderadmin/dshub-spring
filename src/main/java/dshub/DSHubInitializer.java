...
@Service
public class DSHubInitializer extends Thread {
    ...
    public static void PopMsg(String bla) {
        ...
        if (DSHubInitializer.GUIok)
            DSHubInitializer.GUI.insertLog(bla);
        Date d = new Date(DSHubInitializer.curtime);
        if (Vars.savelogs == 1) {
            File logTest = new File((DSHubInitializer.myPath.equals("") ? "" : (DSHubInitializer.myPath + "/")) + "log");
            boolean result = true;
            if (!logTest.exists())
                result = logTest.mkdir();
            if (result) {
                File logFile = new File((DSHubInitializer.myPath.equals("") ? "" : (DSHubInitializer.myPath + "/")) + "log/" + d.toString().replaceAll(" ", "_").replaceAll(":", "_") + ".log");
    ...
    public static void Exit() {
        ...
        listaBanate.printFile(DSHubInitializer.myPath + "banwlist.txt");
    ...
    public void run() {
        ...
        DSHubInitializer.Server.rewritebans();
        DSHubInitializer.Server.restart = true;
        ...
        DSHubInitializer.Server = new HubServer();
        //   System.out.println("ok3");
        DSHubInitializer.curtime = System.currentTimeMillis();
        DSHubInitializer.Proppies = System.getProperties();
    }

    public static void Restart() {
        new DSHubInitializer().start();
        ...
        if (aux.length() == 39) //possible CID, lets try
        {
            try {
                ...
                if (AccountsConfig.isReg(aux) > 0) {
                    ...
                    if (DSHubInitializer.GUIok)
                        DSHubInitializer.GUI.SetStatus(Translation.getString("already_regged"), JOptionPane.WARNING_MESSAGE);
                    return;
                }
                ...
                for (ClientNod temp : SimpleHandler.getUsers()) {
                    if (temp.cur_client.userok == 1) if ((temp.cur_client.ID.equals(aux))) {
                        ...
                        if (DSHubInitializer.GUIok)
                            DSHubInitializer.GUI.SetStatus(Translation.getFoundCid(temp.cur_client.NI));
                ...
                if (DSHubInitializer.GUIok)
                    DSHubInitializer.GUI.SetStatus(Translation.getString("regged_cid_status"));
                ...
                for (ClientNod temp : SimpleHandler.getUsers()) {
                    if (temp.cur_client.userok == 1) if ((temp.cur_client.NI.toLowerCase().equals(aux.toLowerCase()))) {
                        ...
                        if (DSHubInitializer.GUIok)
                            DSHubInitializer.GUI.SetStatus(Translation.getString("found_user"));
                        DSHubInitializer.Server.rewriteregs();
                        return;
                    }
                }
                ...
                if (DSHubInitializer.GUIok)
                    DSHubInitializer.GUI.SetStatus(Translation.getString("no_cid_no_user"), JOptionPane.WARNING_MESSAGE);
    ...
    public static void runCLI(String[] args) {
        ...
        DSHubInitializer.listaBanate = new BanWordsList();
        DSHubInitializer.listaBanate.loadFile(DSHubInitializer.myPath + "banwlist.txt");
            ...
            while (true) {
                ...
                if (recvbuf.toLowerCase().equals("help")) {
                    ...
                    if (nogui) {
                        ...
                        ;
                    } else if (!DSHubInitializer.GUI.isDisplayable()) {
                        try {
                            DSHubInitializer.GUI = new TestFrame();
                            DSHubInitializer.GUIok = true;
                            DSHubInitializer.GUIshowing = true;
                            DSHubInitializer.GUI.SetStatus(Translation.getString("gui_restored"));

                        } catch (Exception e) {
                            System.out.println(Translation.getString("gui_not_viewable"));
                            DSHubInitializer.GUIok = false;
                        }
                    }
                    if (GUIok)
                        if (GUI.isDisplayable() && !GUI.isShowing()) {
